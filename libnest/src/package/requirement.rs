...
impl From<HardPackageRequirement> for SoftPackageRequirement {
    fn from(package_req: HardPackageRequirement) -> Self {
        let (repository, category, name) = package_req.full_name.into_tuple();
        let version_requirement = package_req.version_requirement;
