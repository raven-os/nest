...
lazy_static! {
    ...
    static ref NEST_PATH_DOWNLOAD: &'static Path = Path::new("/var/nest/downloaded/");
...
impl ConfigPaths {
    ...
    pub(crate) fn new() -> ConfigPaths {
        ConfigPaths {
            ...
            downloaded: PathBuf::from(*NEST_PATH_DOWNLOAD),
    ...
    /// Returns a reference to the file's path where the dependecy graph is stored
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// Returns a mutable reference to the file's path where the dependecy graph is stored
    ///
    /// # Examples
    ///
    /// ```no_run
