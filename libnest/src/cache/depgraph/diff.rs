...
use cache::depgraph::{ROOT_ID, NodeId, NodeKind, DependencyGraph};
...
impl DependencyGraphDiff {
    ...
    fn diff_nodes(&mut self, left_graph: &DependencyGraph, right_graph: &DependencyGraph, node_id: NodeId) -> Result<(), Error> {
        // Do not treat nodes that has already been treated.
        if self.taint.contains(&node_id) {
            return Ok(())
        }
        self.taint.insert(node_id);
        match (left_graph.nodes.get(&node_id), right_graph.nodes.get(&node_id)) {
            (Some(left_node), None) => { // Removed nodes
                ...
                for requirement_id in &left_node.requirements {
                    self.diff_nodes(left_graph, right_graph, left_graph.requirements[requirement_id].fulfiller())?;
                }
            },
            (None, Some(right_node)) => { // Added nodes
                // Repeat on dependencies
                for requirement_id in &right_node.requirements {
                    self.diff_nodes(left_graph, right_graph, right_graph.requirements[requirement_id].fulfiller())?;
                }
                // Create a transaction only if the node is a package
            ...
            (Some(left_node), Some(right_node)) => { // Present in both graph (maybe update, maybe nothing)
    ...
    pub fn perform(mut self, left_graph: &DependencyGraph, right_graph: &DependencyGraph) -> Result<Vec<Box<Transaction>>, Error> {
