...
assert nest(chroot="chroot", config="/non_existent/not_existing_either.toml").pull().returncode == 1
assert finest(chroot="chroot", config="/non_existent/not_existing_either.toml").pull().returncode == 1

with create_config(entries={}) as config_path:
    os.chmod(path=config_path, mode=0o222)  # Make the configuration file write-only
    assert nest(chroot="chroot", config=config_path).pull().returncode == 1
    assert finest(chroot="chroot", config=config_path).pull().returncode == 1
